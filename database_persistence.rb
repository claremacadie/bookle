...
class DatabasePersistence
  ...
  def owner_id(book_id)
    ...
    query(sql, book_id).first['owner_id'].to_i
  end
  ...
  def filter_books(title, author, categories, availabilities, limit, offset)
    ...
    offset_clause = "OFFSET #{offset}"
    sql = [
      select_clause, where_clause_filter(categories, availabilities),
  ...
  def availabilities_clause(availabilities)
    ...
    when 'available,on_loan' then ' AND books.requester_id IS NULL'
    when 'requested,on_loan' then ' AND (books.requester_id IS NOT NULL OR books.borrower_id IS NOT NULL)'
    when 'available' then ' AND books.borrower_id IS NULL AND books.requester_id IS NULL'
    when 'requested' then ' AND books.requester_id IS NOT NULL'
    when 'on_loan' then ' AND books.borrower_id IS NOT NULL'
    else ''
    end
  end
